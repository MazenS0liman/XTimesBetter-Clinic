...
//Authenticate part
  ...
  useEffect(() => {
      ...
      axios ({
        method: 'get',
        url: 'http://localhost:5000/patient/Video/AllApointments',
        headers: {
            "Content-Type": "application/json",
            'Authorization': accessToken,
        },
  ...
  const handleStartClick = async (appointID) => {
    ...
    try {
        const apiUrl = 'http://localhost:5000/patient/Video/start';
        ...
        if (response.ok) {
          console.log(response)
          const apiUrl2 = `http://localhost:5000/patient/google/google?username=${appointID.doctor_username}&type=doctor`;
