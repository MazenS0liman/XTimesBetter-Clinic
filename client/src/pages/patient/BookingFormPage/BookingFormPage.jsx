...
const BookAppointmentForm = () => {
    ...
    useEffect(() => {
        const fetchLinkedFamilyMembers = async () => {
            const response = await fetch(`http://localhost:5000/patient/viewFamilyMembers/LinkedFamilyMembers?username=${patient_username}`);
            const familyMembers = await response.json();
            if (response.ok) {
    ...
    const submitAppointment = async () => {
        ...
        const appointmentData = {
            ...
            booked_by: appointment.currentPatient
        };
